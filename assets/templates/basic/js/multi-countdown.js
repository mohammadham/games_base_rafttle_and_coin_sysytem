...
$(function () {
    ...
    $(mainClass).each(function () { //for each countdown instance
        ...
        if ($('.' + extraClass + ' ' + runningClass + ' ul').length) {
            initText = $('.' + extraClass + ' ' + runningClass + ' ul').text();
        } else {
            initText = $(this).text();
        }
    ...
    function dateReplace(extraClass, date, fixTime, initText, zeroPad) {
        ...
        if (dif[0] < 0 || dif[1] < 0 || dif[2] < 0 || dif[3] < 0) {
           ...
           if(zeroPad != undefined) zeroPadArr = JSON.parse(zeroPad);
            ...
            //whith extras Class
            if ($('.' + extraClass + ' ' + runningClass + ' ul').length) {
                $('.' + extraClass + ' ' + runningClass + ' ul .days').text(dif[0]);
                $('.' + extraClass + ' ' + runningClass + ' ul .hours').text(dif[1]);
                $('.' + extraClass + ' ' + runningClass + ' ul .minutes').text(dif[2]);
                $('.' + extraClass + ' ' + runningClass + ' ul .seconds').text(dif[3]);
